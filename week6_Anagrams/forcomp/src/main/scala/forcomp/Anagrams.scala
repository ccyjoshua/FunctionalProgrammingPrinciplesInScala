...
object Anagrams extends AnagramsInterface:
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    if occurrences.isEmpty then
      List[Occurrences](Nil)  // note: not just Nil
    else
      occurrences match
        case o :: os =>
          val osComb = combinations(os)  // combination of tail (sub-problem)
          // add every occurrence of head to sub-problem
          val oComb = for i <- (1 to o._2).toList
                          osOcc <- osComb
          yield (o._1, i) :: osOcc
          oComb ++ osComb
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    ...
    x.map{ case (c, o) => (c, o - yMap(c))}.filter(_._2 != 0)
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] =
    def helper(occ: Occurrences): List[Sentence] =
      if occ.isEmpty then
        ...
        for
          sub <- combinations(occ)
          w <- dictionaryByOccurrences(sub)
          sen <- helper(subtract(occ, sub))
        yield w :: sen
